...
namespace Player.Effects
{
    public class SpeedLinesVFX : MonoBehaviour
    {
        ...
        private void Update()
        {
            float v = _playerCharacterController.GetRigidbody().velocity.magnitude;
            ...
            if (v > thresholdMin)
            {
                ...
                if (_playerCharacterController.GetMovementHandler() is ChargeMovementHandler)
                {
                    vfx.SetGradient("ColorGradient", chargeGradient);
                }
                else if (_playerCharacterController.GetMovementHandler() is HookMovementHandler)
                {
                    vfx.SetGradient("ColorGradient", hookGradient);
                }
                else {
                    vfx.SetGradient("ColorGradient", defaultGradient);
                }
            }
        }
    }
