...
public class AirborneMovementHandler : MovementHandler
{
    ...
    public void Move(PlayerCharacterController player)
    {
        ...
        if (Physics.CheckCapsule(p1, p2, playerCollider.radius + 0.075f, GameManager.Instance.GetRaycastLayersWithoutAlly(), QueryTriggerInteraction.Ignore) && Physics.CapsuleCast(p1, p2, playerCollider.radius, direction, out hit, playerCollider.radius + 1f, GameManager.Instance.GetRaycastLayers(), QueryTriggerInteraction.Ignore))
        {
